...
APP_BUILD = "Codex 2026-05-19 history search fast"
    ...
    (function() {
        ...
        doc.querySelectorAll('[data-codex-add-sticky="1"]').forEach(function(part) {
            ...
            [
                ...
                'margin','border','border-radius','width','max-width','left','right'
            ...
            if (addMarker) {
                ...
                if (lotBlock && markerChild) {
                    ...
                    formParts.forEach(function(part) {
                        ...
                        part.style.setProperty('padding', '0.25rem 1rem', 'important');
                        part.style.setProperty('margin', '0', 'important');
                        part.style.setProperty('border-left', '2px solid #111827', 'important');
                        part.style.setProperty('border-right', '2px solid #111827', 'important');
                        part.querySelectorAll('button').forEach(function(btn) {
                            btn.style.setProperty('width', '100%', 'important');
                        });
                        if (part === formParts[0]) {
                            part.style.setProperty('border-top', '2px solid #111827', 'important');
                            part.style.setProperty('border-radius', '12px 12px 0 0', 'important');
                        }
                        if (part === formParts[formParts.length - 1]) {
                            part.style.setProperty('border-bottom', '2px solid #111827', 'important');
...
# ============================================================
    ...
    if not hist_enriched:
        ...
        if search_hist:
            ...
            filtered = [
                h for h in filtered
                if search_hist_norm in normalize_name(" ".join([
                    str(h.get("card_name", "")),
                    str(h.get("lot_name", "")),
                    str(h.get("card_set", "")),
                    str(h.get("card_number", "")),
                    str(h.get("canal", "")),
                ]))
            ]
        if filter_month:
        ...
        show_all_hist = st.checkbox(
            ...
            value=False,
            key="show_all_history",
            help="Désactivé par défaut pour garder le menu fluide."
        )
        visible_history = filtered if show_all_hist or search_hist else filtered[:80]
        if len(visible_history) < len(filtered):
            st.caption(f"Affichage rapide : {len(visible_history)} vente(s) sur {len(filtered)}. Utilise la recherche ou coche tout afficher.")
        st.markdown("---")
