...
def _get_sale_lot_grid_component():
    global _sale_lot_grid_component
    if components_v2 is None:
        return None
    if _sale_lot_grid_component is not None:
        return _sale_lot_grid_component
    ...
    export default function(component) {
        ...
        const key = data.key || "sale_lots";
        const stateKey = "__pokestockSaleLotGrid_" + key;
        const groups = Array.isArray(data.groups) ? data.groups : [];
        const cart = new Set(Array.isArray(data.cartUids) ? data.cartUids : []);
        const signature = String(data.signature || "");
        const cartSignature = JSON.stringify(Array.from(cart).sort());
        const scrollTopToken = String(data.scrollTopToken || "");
        const gap = 10;
        const lotGap = 14;
        const headerHeight = 42;
        const qtyState = win[stateKey] || {
            signature: signature,
        ...
        if (qtyState.signature !== signature) {
            qtyState.signature = signature;
            qtyState.cartSignature = cartSignature;
            qtyState.qty = {};
            qtyState.mounted = new Map();
            qtyState.lastRange = "";
            qtyState.layout = null;
            qtyState.preloaded = new Map();
            stage.replaceChildren();
        } else if (qtyState.cartSignature !== cartSignature) {
            qtyState.cartSignature = cartSignature;
            qtyState.mounted = new Map();
            qtyState.lastRange = "";
            stage.replaceChildren();
        ...
        function layoutFor(colCount, rowHeight) {
            if (
                qtyState.layout &&
                qtyState.layout.signature === signature &&
                qtyState.layout.colCount === colCount &&
                qtyState.layout.rowHeight === rowHeight
            ) {
                return qtyState.layout;
            }
        ...
        function makeCard(item, width, height) {
            const key = itemKey(item);
            const card = doc.createElement("div");
            card.className = "ps-sale-lot-card" + (cart.has(key) ? " in-cart" : "");
            card.dataset.cardKey = key;
            card.style.width = width + "px";
            card.style.height = height + "px";
            ...
            action.textContent = cart.has(key) ? "Dans panier" : "Ajouter";
            action.onclick = (event) => {
                event.preventDefault();
                setTriggerValue("action", {
                    id: "sale-" + (cart.has(key) ? "remove" : "add") + "-" + key + "-" + Date.now() + "-" + Math.random().toString(36).slice(2),
                    type: cart.has(key) ? "remove" : "add",
                    card_uid: item.card_uid,
                    lot_uid: item.lot_uid,
                    lot_idx: item.lot_idx,
                    card_idx: item.card_idx,
                    quantity: clampQty(key, item.stock),
                    lot_profitable: Boolean(item.lot_profitable)
                });
            };
        ...
        function makeCardRow(row, cardWidth, rowHeight, colCount) {
            const el = doc.createElement("div");
        ...
        function preload(rows, start, end) {
            ...
            for (let i = Math.max(0, start); i < Math.min(rows.length, end); i++) {
                ...
                row.cards.forEach((item) => {
                    ...
                    if (qtyState.preloaded.has(item.image_url)) {
                        const cached = qtyState.preloaded.get(item.image_url);
                    ...
                    try {
                        ...
                        img.loading = "eager";
                        img.src = item.image_url;
                        qtyState.preloaded.set(item.image_url, { time: now, img });
                        loaded += 1;
                    } catch (e) {}
                });
            }
            if (qtyState.preloaded.size > 96) {
                const stale = Array.from(qtyState.preloaded.entries())
                    .sort((a, b) => (a[1].time || 0) - (b[1].time || 0))
                    .slice(0, qtyState.preloaded.size - 96);
                stale.forEach(([url]) => qtyState.preloaded.delete(url));
            }
        }
        ...
        function runScrollTopAfterLayout() {
            const consumedTokenKey = "__pokestockSaleScrollTopConsumedToken";
        ...
        function render() {
            ...
            if (rangeKey === qtyState.lastRange && stage.childElementCount) {
            ...
            for (let index = start; index < end; index++) {
                ...
                if (!row) continue;
                keep.add(row.key);
                let node = qtyState.mounted.get(row.key);
                if (!node) {
                    node = row.type === "header"
                        ? makeHeader(row)
                        : makeCardRow(row, cardWidth, rowHeight, colCount);
                    qtyState.mounted.set(row.key, node);
            ...
            for (const [rowKey, node] of Array.from(qtyState.mounted.entries())) {
                if (!keep.has(rowKey)) {
                    node.remove();
                    qtyState.mounted.delete(rowKey);
                }
            }
            preload(rows, end, end + overscan.after);
            root.dataset.computedFontFamily = win.getComputedStyle(root).fontFamily;
        ...
        if (qtyState.scrollListener && qtyState.scrollTarget) {
            try { qtyState.scrollTarget.removeEventListener("scroll", qtyState.scrollListener); } catch (e) {}
        }
        if (qtyState.resizeListener) {
            try { win.removeEventListener("resize", qtyState.resizeListener); } catch (e) {}
        }
        scrollTarget.addEventListener("scroll", schedule, { passive: true });
        win.addEventListener("resize", schedule, { passive: true });
        qtyState.scrollTarget = scrollTarget;
        qtyState.scrollListener = schedule;
        qtyState.resizeListener = schedule;
    ...
    _sale_lot_grid_component = components_v2.component(
        "pokestock_sale_virtual_lot_grid",
        html=(
...
def grouped_payload_signature(groups):
    compact = []
    for group in groups or []:
        compact.append((
            ...
            group.get("lot_idx"),
            [
                (item.get("card_uid"), item.get("card_idx"), item.get("stock"), item.get("price"))
...
def _estimated_lot_grid_height(groups, *, mobile=False):
    cols = 2 if mobile else 6
...
def render_sale_virtual_lot_grid(groups, cart_uids, *, key="sales", height=None, mobile=False, scroll_top_token=0):
    component = _get_sale_lot_grid_component()
    if component is None:
        return None
    ...
    result = component(
        key=f"sale_virtual_lot_grid_{key}",
        data={
            "key": key,
            "signature": grouped_payload_signature(groups),
