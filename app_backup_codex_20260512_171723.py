...
def _scu_in_data(cd, li, ci, q, p, canal="Main propre"):
    ...
    if crd["quantity"]<crd.get("sold_quantity",0)+q:
...
def scu_many(items, canal="Main propre"):
    ...
    for (lot_idx, card_idx), qty in requested.items():
        ...
        if crd["quantity"] < crd.get("sold_quantity", 0) + qty:
...
def bulk_cart_add(item):
    ...
    stock = int(card.get("quantity", 0)) - int(card.get("sold_quantity", 0))
...
def transfer_card_to_storage(li, ci, qty):
    ...
    available = int(source_card.get("quantity", 0)) - int(source_card.get("sold_quantity", 0))
    ...
    source_card["quantity"] = int(source_card.get("quantity", 0)) - qty
    if source_card["quantity"] <= int(source_card.get("sold_quantity", 0)):
        source_card["quantity"] = int(source_card.get("sold_quantity", 0))
    if source_card["quantity"] == 0:
        cd["lots"][li]["cards"].pop(ci)
        ...
        if not cd.get("lots"):
            ...
            if search_vente:
                ...
                for li, lot in enumerate(cd["lots"]):
                    if selected_lot_idx is not None and li != selected_lot_idx:
                        continue
                    for ci, card in enumerate(lot.get("cards", [])):
                        if card['quantity'] > card.get('sold_quantity', 0) and normalize_name(search_vente) in normalize_name(card.get("name","")):
                ...
                for li, lot in enumerate(cd["lots"]):
                    if selected_lot_idx is not None and li != selected_lot_idx:
                        continue
                    cards_in_stock = [(ci, c) for ci, c in enumerate(lot.get("cards", [])) if c['quantity'] > c.get('sold_quantity', 0)]
                    if cards_in_stock:
            ...
            if not st.session_state.bulk_cart:
                ...
                for idx, item in enumerate(st.session_state.bulk_cart):
                    ...
                    cols[1].number_input("Qté", 1, 999, int(item["quantity"]), key=f"cart_qty_{idx}", on_change=bulk_cart_set_quantity, args=(idx,), label_visibility="collapsed")
                        ...
                        if c1.button("✅ Confirmer", type="primary", use_container_width=True):
                            if pending.get("type") == "base":
                                ...
                                def get_lot_score(lot_idx):
                                    ...
                                    ca = cr(lot_data)
                                    stock_val = sum((c.get("quantity",0)-c.get("sold_quantity",0))*c.get("suggested_price",0.) for c in lot_data.get("cards",[]))
    ...
    if not cd.get("lots"):
        ...
        for display_ix,(ix,lt) in enumerate(visible_lots):
            ...
            with st.expander(expander_title, expanded=st.session_state.get(f"lot_expanded_{ix}", False)):
                ...
                cards_with_idx = [(i, c) for i, c in enumerate(cards_all)]
                cards_in_stock_lot = [(i, c) for i, c in cards_with_idx if c['quantity'] - c.get('sold_quantity', 0) > 0]
                cards_sold_lot    = [(i, c) for i, c in cards_with_idx if c['quantity'] - c.get('sold_quantity', 0) <= 0]
                ...
                if not show_all_cards:
                    ...
                    hidden_cards_count = max(len(cards_in_stock_lot) - len(visible_stock_lot), 0) + max(len(cards_sold_lot) - len(visible_sold_lot), 0)
    ...
    for lot_idx_s, lot in enumerate(all_lots + archives_list):
        ...
        for v in lot.get("ventes", []):
            ...
            try:
                ...
                cout_v = (price / (valeur_est or 1.0)) * float(lot.get("prix_achat", 0.))
                all_sales.append({
...
# ============================================================
    ...
    with col_mb:
        ...
        total_mb_ca = cnt_main_brocante["ca"] + float(counters["main_brocante"].get("init_ca", 0.))
        st.markdown(f"""
    ...
    with col_dx:
        total_dx_ca = cnt_dexify["ca"] + float(counters["dexify"].get("init_ca", 0.))
        st.markdown(f"""
        <div style="background:white;border-radius:16px;padding:1.5rem;border:2px solid #e2e8f0;
                    box-shadow:0 2px 8px rgba(0,0,0,0.06);text-align:center;">
    ...
    with col_pk:
        total_pk_ca = cnt_pokedeal["ca"] + float(counters["pokedeal"].get("init_ca", 0.))
        st.markdown(f"""
        <div style="background:white;border-radius:16px;padding:1.5rem;border:2px solid #e2e8f0;
                    box-shadow:0 2px 8px rgba(0,0,0,0.06);text-align:center;">
    ...
    with st.expander("⚙️ Données de départ (à saisir une seule fois)"):
        ...
        mb_init_ca = vi1.number_input("🤝 Main propre & Brocante — CA (€)", 0., 999999., init_mb_ca, key="init_mb_ca")
        dx_init_ca = vi2.number_input("⚡ Dexify_TCG — CA (€)", 0., 999999., init_dx_ca, key="init_dx_ca")
        pk_init_ca = vi3.number_input("🎴 Pokédeal — CA (€)", 0., 999999., init_pk_ca, key="init_pk_ca")
