...
APP_BUILD = "Codex 2026-05-18 18:50"
...
with st.sidebar:
    ...
    if st.button("Accueil", use_container_width=True, key="nav_accueil", type="primary" if st.session_state.current_page == "Accueil" else "secondary"):
        set_current_page("Accueil")
        st.rerun()
    if st.button("Vente/Échange", use_container_width=True, key="nav_vente", type="primary" if st.session_state.current_page == "Vente" else "secondary"):
        set_current_page("Vente")
        st.rerun()
    if st.button("Lots", use_container_width=True, key="nav_lots", type="primary" if st.session_state.current_page == "Lots" else "secondary"):
        set_current_page("Lots")
        st.rerun()
    if st.button("Historique", use_container_width=True, key="nav_historique", type="primary" if st.session_state.current_page == "Historique" else "secondary"):
        set_current_page("Historique")
        st.rerun()
    if st.button("📊 Statistiques", use_container_width=True, key="nav_statistiques", type="primary" if st.session_state.current_page == "Statistiques" else "secondary"):
        set_current_page("Statistiques")
        st.rerun()
    if st.button("🎰 Compteurs", use_container_width=True, key="nav_compteurs", type="primary" if st.session_state.current_page == "Compteurs" else "secondary"):
        set_current_page("Compteurs")
        st.rerun()
    if st.button("Archivés", use_container_width=True, key="nav_archives", type="primary" if st.session_state.current_page == "Archivés" else "secondary"):
        set_current_page("Archivés")
        st.rerun()
...
# ============================================================
    ...
    if not cd.get("lots"):
        ...
        filter_cols = st.columns(len(filter_defs))
        for fcol, (slug, name, _) in zip(filter_cols, filter_defs):
            label = f"{name} ({filter_counts[slug]})"
            btn_type = "primary" if slug == selected_filter_slug else "secondary"
            if fcol.button(label, key=f"lot_filter_{slug}", use_container_width=True, type=btn_type):
