...
APP_BUILD = "Codex 2026-05-18 restore lots"
...
# ============================================================
    ...
    # Bordures via emoji dans le titre - passage ponctuel pour ne pas gêner les clics Streamlit
    ...
    if not cd.get("lots"):
        ...
        for display_ix,(ix,lt) in enumerate(visible_lots):
            ...
            with st.expander(expander_title, expanded=st.session_state.get(f"lot_expanded_{ix}", False)):
                detail_label = "Masquer le détail du lot" if is_active_lot else "Ouvrir / modifier ce lot"
                if st.button(detail_label, key=f"open_lot_detail_{ix}", use_container_width=True):
                    if is_active_lot:
                ...
                if not is_active_lot:
                    st.caption("Le détail de ce lot se charge uniquement quand tu l'ouvres, pour garder la page rapide.")
                    continue
