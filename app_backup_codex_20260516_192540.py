...
# ============================================================
    ...
    with tab2:
        ...
        if not cd.get("lots"):
            ...
            with col_lot_filter:
                lot_options = [("Tous les lots", None)] + [(f"{i+1}. {lot.get('nom', f'Lot {i+1}')}", i) for i, lot in enumerate(cd.get("lots", [])) if not is_storage_lot(lot)]
            ...
            if not search_vente and selected_lot_idx is None:
                st.info("Recherche une carte ou choisis un lot pour afficher les cartes. Ça évite de charger toutes les images d'un coup.")
            elif search_vente:
