...
def calc_cout_lot(lot, valeur_estimee=None, lot_idx=None):
    """
    coût_vente = (prix_vendu / valeur_estimee_lot) × prix_achat_lot
    valeur_estimee_lot = stock_actuel (suggested_price) + CA_réel_lot
    ...
    if valeur_estimee is None or valeur_estimee <= 0:
        stock_val = sum(
            float(c.get("suggested_price", 0.)) * max(int(c.get("quantity", 1)) - int(c.get("sold_quantity", 0)), 0)
            for c in cards
        )
        ca_reel = sum(float(se.get("price", 0.)) for c in cards for se in c.get("sold_entries", []))
        ca_reel += sum(float(v.get("price", 0.)) for v in lot.get("ventes", []) if not v.get("is_exchange_benefit"))
        valeur_estimee = stock_val + ca_reel
    ...
    for card in cards:
        for se in card.get("sold_entries", []):
            ...
            if lot.get("is_divers") and card.get("purchase_price"):
                cout = float(card["purchase_price"]) * int(se.get("quantity", 1))
            # Carte reçue par échange avec repartition connue
                ...
                cout = (price / valeur_estimee) * prix_lot
...
def acm_japanese(li, n, sn, num, q, co, p, ir, ie, purchase_price=0.):
...
def acm(li,n,sn,num,q,co,p,ir,ie,lang="fr",purchase_price=0.):
    ...
    if lang == "ja":
        return acm_japanese(li, n, sn, num, q, co, p, ir, ie, purchase_price=purchase_price)
    ...
    if len(multi)>1:
        ...
        for nm in multi:
            if nm:
                lok,lmg=acm(li,nm,sn,num,q,co,p,ir,ie,lang=lang,purchase_price=purchase_price)
                if lok:
                    ok_count+=1
...
def transfer_card_to_storage(li, ci, qty):
    cd = ld()
...
<style>
    ...
    .stRadio input:checked + div {
        ...
        border-color: var(--pokemon-blue) !important;
        color: white !important;
    }
...
with st.sidebar:
    ...
    st.button("Accueil",use_container_width=True,on_click=set_current_page,args=("Accueil",))
    st.button("Vente/Échange",use_container_width=True,on_click=set_current_page,args=("Vente",))
    st.button("Lots",use_container_width=True,on_click=set_current_page,args=("Lots",))
    st.button("Historique",use_container_width=True,on_click=set_current_page,args=("Historique",))
    st.button("📊 Statistiques",use_container_width=True,on_click=set_current_page,args=("Statistiques",))
    st.button("🎰 Compteurs",use_container_width=True,on_click=set_current_page,args=("Compteurs",))
    st.button("Archivés",use_container_width=True,on_click=set_current_page,args=("Archivés",))
...
# ============================================================
    ...
    components.html("<script>setTimeout(()=>parent.window.scrollTo({top:0,left:0,behavior:'instant'}),50);</script>", height=0)
    ...
    if False:
        cd=ld()
        if not cd.get("lots"):
            st.warning("Créez d'abord un lot dans l'onglet Lots")
        else:
            search_rapide = st.text_input("🔍 Rechercher une carte dans tous les lots", 
                                          placeholder="Nom de la carte...", 
                                          key="search_vente_rapide")

            # Collecter toutes les cartes en stock de tous les lots
            all_cards_stock = []
            for li, lot in enumerate(cd["lots"]):
                for ci, card in enumerate(lot.get("cards", [])):
                    stock = card_available_qty(card)
                    if stock > 0:
                        all_cards_stock.append((li, ci, card, lot, stock))

            # Filtrer par recherche
            if search_rapide:
                all_cards_stock = [
                    (li, ci, c, lot, stock)
                    for li, ci, c, lot, stock in all_cards_stock
                    if normalize_name(search_rapide) in normalize_name(c.get("name", ""))
                ]

            if not all_cards_stock:
                st.info("Aucune carte en stock." if not search_rapide else f"Aucune carte trouvée pour « {search_rapide} »")
            else:
                st.markdown(f"**{len(all_cards_stock)} carte(s) en stock**")
                COLS_PER_ROW = 6

                if search_rapide:
                    # Recherche active → grille simple sans séparation par lot
                    for row_start in range(0, len(all_cards_stock), COLS_PER_ROW):
                        cols = st.columns(COLS_PER_ROW)
                        for col_idx, (li, ci, card, lot, stock) in enumerate(all_cards_stock[row_start:row_start + COLS_PER_ROW]):
                            with cols[col_idx]:
                                ...
                                q = st.number_input("Qté", 1, stock, 1, key=f"vr_q_{li}_{ci}")
                                p = st.number_input("Prix €", 0., 9999., float(card.get("suggested_price", 0.)), 0.5, key=f"vr_p_{li}_{ci}")
                                if st.button("💰 Vendre", key=f"vr_sell_{li}_{ci}", use_container_width=True, type="primary"):
                                    ask_canal(li, ci, q, p)
                else:
                    # Pas de recherche → grouper par lot avec titre au dessus
                    from itertools import groupby
                    cards_by_lot = {}
                    for li, ci, card, lot, stock in all_cards_stock:
                        lot_key = (li, lot["nom"])
                        cards_by_lot.setdefault(lot_key, []).append((li, ci, card, lot, stock))

                    for (li, lot_nom), cards_lot in cards_by_lot.items():
                        st.markdown(f"#### 🗂️ {lot_nom}")
                        for row_start in range(0, len(cards_lot), COLS_PER_ROW):
                            cols = st.columns(COLS_PER_ROW)
                            for col_idx, (li, ci, card, lot, stock) in enumerate(cards_lot[row_start:row_start + COLS_PER_ROW]):
                                with cols[col_idx]:
                                    if card.get("image_url"):
                                        st.image(proxy_img(card["image_url"]), width="stretch")
                                    else:
    ...
    with tab2:
        ...
        if not cd.get("lots"):
            ...
            with col_lot_filter:
                lot_options = [("Tous les lots", None)] + [(f"{i+1}. {lot.get('nom', f'Lot {i+1}')}", i) for i, lot in enumerate(cd.get("lots", []))]
                lot_labels = [name for name, _ in lot_options]
                selected_lot_label = st.selectbox("Lot affiché", lot_labels, key="bulk_lot_filter", label_visibility="collapsed")
...
# ============================================================
    ...
    # Bordures via emoji dans le titre - méthode 100% fiable
    components.html("""<script>
    (function(){
        function colorLotBorders() {
            ...
            markers.forEach(function(marker) {
                const idx = parseInt(marker.getAttribute('data-display-index') || marker.getAttribute('data-lot-index')) + 1;
                const status = marker.getAttribute('data-lot-status');
                if (!allExpanders[idx]) return;
                let color = '#22c55e';
                ...
                allExpanders[idx].style.setProperty('border-left', '8px solid ' + color, 'important');
                allExpanders[idx].style.setProperty('border-radius', '8px', 'important');
            });
        }
        setTimeout(colorLotBorders, 300);
        setTimeout(colorLotBorders, 800);
        setInterval(colorLotBorders, 2000);
    })();
    </script>""", height=0)
    ...
    if not cd.get("lots"):
        ...
        lots_with_idx = list(enumerate(cd["lots"]))
        completed_lots = [
        ...
        filter_defs = [
            ...
            ("Trade", lambda item: is_trade_lot(item[1])),
            ("Stockage", lambda item: is_storage_lot(item[1])),
        ]
        ...
        for display_ix,(ix,lt) in enumerate(visible_lots):
            ...
            if f"lot_expanded_{ix}" not in st.session_state:
                st.session_state[f"lot_expanded_{ix}"] = False
            with st.expander(expander_title, expanded=st.session_state.get(f"lot_expanded_{ix}", False)):
                ...
                if lt.get("is_mixte") and lt.get("valeur_totale", 0) > 0:
                    valeur_vente_auto = sum(
                        float(c.get("suggested_price", 0.)) * int(c.get("quantity", 1))
                        for c in lt.get("cards", [])
                    )
                    # Ajouter aussi les cartes vendues (leur prix de vente)
                    valeur_vente_auto += sum(
                        float(se.get("price", 0.))
                        for c in lt.get("cards", [])
                        for se in c.get("sold_entries", [])
                    )
                ...
                setTimeout(() => {{
                    const doc = parent.document;
                    const marker = doc.querySelector('[data-sticky-add-lot="{ix}"]');
                    const markerBlock = marker?.closest('[data-testid="stMarkdownContainer"]')?.parentElement;
                    ...
                    if (target) {{
                        ...
                        target.style.padding = '0.5rem 0';
                    }}
                }}, 300);
                </script>
                """, height=0)
                st.markdown(f'<span data-sticky-add-lot="{ix}"></span>', unsafe_allow_html=True)
                ...
                if st.button("Ajouter",key=f"ad{ix}",disabled=st.session_state.get("searching",False)):
                    ...
                    final_qt=qt
                    final_pi=pi
                    ok,mg=acm(ix,nm,sn,nu,final_qt,cn,final_pi,rv_check,ed,lang="ja" if is_jp else "fr",purchase_price=pa_divers if is_divers_lot else 0.)
                            ...
                            with cols[col_idx]:
                                ...
                                if sold and crd.get("sold_entries"):
                                    ...
                                    if ph and len(ph) >= 2:
                                        ...
                                        previous_price = float(ph[-2]["price"])
                                        if st.button("↩️ Prix précédent", key=f"restore_price_{ix}_{real_cix}", use_container_width=True):
                                            cdd = ld()
                                            cdd["lots"][ix]["cards"][real_cix]["suggested_price"] = previous_price
                                            cdd["lots"][ix]["cards"][real_cix].setdefault("price_history", []).append({
                                                "date": datetime.now().isoformat()[:10],
                                ...
                                if not sold:
                                    ...
                                    if (not is_storage) and stock > 0:
                                        ...
                                        if st.button("📈 Stocker", key=f"store_btn_{ix}_{real_cix}", use_container_width=True):
                                            ok, msg = transfer_card_to_storage(ix, real_cix, transfer_qty)
                                            if ok:
                                                st.success(msg)
                                                st.rerun()
                                            else:
                                ...
                                # Vendre
                                if not sold and stock > 0:
                                    prix_vente_input = float(crd.get("suggested_price") or 0)
                                    if st.button("💰 Vendre", key=f"sell{ix}_{real_cix}", use_container_width=True, type="primary"):
                                        ask_canal(ix, real_cix, 1, prix_vente_input)
...
# ============================================================
    ...
    for lot_idx_h, lot in enumerate(all_lots_hist + archives_hist):
        ...
        for v in lot.get("ventes",[]):
            ...
            cout_v = (price_v / (valeur_est_hist or 1.0)) * prix_lot
    ...
    if not hist_enriched:
        ...
        search_hist = col_search.text_input("🔍 Rechercher une carte", placeholder="Nom...")
        ...
        if search_hist:
            filtered = [h for h in filtered if normalize_name(search_hist) in normalize_name(h.get("card_name",""))]
        if filter_month:
        ...
        for h in filtered:
            ...
            with img_col:
                img = h.get("image_url","")
                if img:
                    st.markdown(f'<img src="{proxy_img(img)}" style="width:60px;border-radius:6px;box-shadow:0 2px 6px rgba(0,0,0,0.12);">', unsafe_allow_html=True)
                else:
            ...
            with info_col:
                ...
                <div style="padding:0.2rem 0;">
                  <div style="font-weight:700;font-size:0.98rem;color:#1e293b;">{h['card_name']}{canal_badge}</div>
                ...
                for row_start in range(0, len(card_list), COLS):
                    ...
                    for col_idx, (real_cix, crd) in enumerate(card_list[row_start:row_start+COLS]):
                        ...
                        with cols_g[col_idx]:
                            ...
                            if not sold and stock > 0:
                                prix_vente_divers = float(crd.get("suggested_price") or 0)
                                if st.button("💰 Vendre",key=f"bsell{ix}_{real_cix}",use_container_width=True,type="primary"):
                                    ask_canal(ix, real_cix, 1, prix_vente_divers)
                            if sold:
...
# ============================================================
    ...
    default_purchase_rate = float(app_settings.get("stats_purchase_rate", 0.60))
    with st.expander("⚙️ Calcul du bénéfice", expanded=False):
        st.caption("Le bénéfice affiché ici utilise un coût d'achat estimé depuis la cote de chaque carte vendue. Exemple : 60% signifie qu'une carte cotée 10€ est estimée achetée 6€.")
        purchase_rate_pct = st.number_input(
            "Pourcentage d'achat moyen par rapport à la cote",
            min_value=0,
            max_value=100,
            value=int(round(default_purchase_rate * 100)),
            step=1,
            key="stats_purchase_rate_pct",
        )
        if st.button("💾 Sauvegarder ce pourcentage", key="save_stats_purchase_rate"):
            app_settings["stats_purchase_rate"] = purchase_rate_pct / 100
            save_app_settings(app_settings)
            st.success("Pourcentage sauvegardé.")
            st.rerun()
    purchase_rate = purchase_rate_pct / 100

    def estimated_card_cost(card, se, price, qty, fallback_cost=0.):
        if card.get("received_by_exchange") and card.get("exchange_repartition"):
            return float(fallback_cost)
        if card.get("purchase_price"):
            return float(card.get("purchase_price", 0.)) * qty
        cote_unit = float(se.get("suggested_price_at_sale", 0.) or card.get("suggested_price", 0.) or 0.)
        cote_total = cote_unit * qty
        if cote_total <= 0:
            cote_total = price
        return cote_total * purchase_rate
    ...
    for lot_idx_s, lot in enumerate(all_lots + archives_list):
        ...
        for card, se, cout_total in ventes_avec_cout:
            try:
                ...
                cout_estime = estimated_card_cost(card, se, price, qty, cout_total)
                ...
                all_sales.append({
                    ...
                    "unit_price": price / max(qty, 1),
                    "cost": cout_estime,
                    "benef": price - cout_estime,
                    "cote": cote_total,
                    "cost_lot_method": cout_total,
                })
            except:
                pass
    ...
    with st.expander("🔎 Détail du bénéfice du mois"):
        ...
        st.caption(f"Calcul actuel : coût estimé = cote × {purchase_rate_pct}%, sauf cartes avec prix d'achat individuel ou échange.")
...
# ============================================================
    ...
    init_mb_display = float(st.session_state.get("init_mb_ca_input", counters["main_brocante"].get("init_ca", 0.)))
    init_dx_display = float(st.session_state.get("init_dx_ca_input", counters["dexify"].get("init_ca", 0.)))
    init_pk_display = float(st.session_state.get("init_pk_ca_input", counters["pokedeal"].get("init_ca", 0.)))
    ...
    for lot in all_lots + archives_cnt:
        for v in lot.get("ventes", []):
            ...
            if not canal:
                continue
            date_str = v.get("date", "")[:10]
            price = float(v.get("price", 0))
            if canal in ("main", "brocante"):
                if date_str >= start_date_mb:
                    cnt_main_brocante["ca"] += price
            elif canal == "dexify":
                if date_str.startswith(dexify_year):
                    cnt_dexify["ca"] += price
            elif canal == "pokedeal":
                if date_str.startswith(pokedeal_year):
    ...
    with col_dx:
        ...
        <div style="background:white;border-radius:16px;padding:1.5rem;border:2px solid #e2e8f0;
          ...
          <div style="font-size:0.75rem;color:#94a3b8;margin-top:0.5rem;">Année {dexify_year}</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col_pk:
        ...
        <div style="background:white;border-radius:16px;padding:1.5rem;border:2px solid #e2e8f0;
          ...
          <div style="font-size:0.75rem;color:#94a3b8;margin-top:0.5rem;">Année {pokedeal_year}</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    st.markdown("---")
    with st.expander("⚙️ Données de départ (à saisir une seule fois)"):
        st.caption("Saisis ici tes données existantes avant l'utilisation de l'appli pour initialiser les compteurs.")
        vi1, vi2, vi3 = st.columns(3)
        init_mb_ca = float(counters["main_brocante"].get("init_ca", 0.))
        init_dx_ca = float(counters["dexify"].get("init_ca", 0.))
        init_pk_ca = float(counters["pokedeal"].get("init_ca", 0.))

        mb_init_ca = vi1.number_input("🤝 Main propre & Brocante — CA (€)", 0., 999999., init_mb_ca, key="init_mb_ca_input")
        dx_init_ca = vi2.number_input("⚡ Dexify_TCG — CA (€)", 0., 999999., init_dx_ca, key="init_dx_ca_input")
        pk_init_ca = vi3.number_input("🎴 Pokédeal — CA (€)", 0., 999999., init_pk_ca, key="init_pk_ca_input")

        if st.button("💾 Sauvegarder", type="primary"):
            counters["main_brocante"]["init_ca"] = mb_init_ca
            counters["dexify"]["init_ca"] = dx_init_ca
            counters["pokedeal"]["init_ca"] = pk_init_ca
            safe_write_json(COUNTERS_FILE, counters)
            st.success("✅ Valeurs initiales sauvegardées !")
            st.rerun()
