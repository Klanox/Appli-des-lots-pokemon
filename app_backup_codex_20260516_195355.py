...
with st.sidebar:
    ...
    if st.button("Accueil", use_container_width=True, key="nav_accueil"):
        ...
        st.rerun()
    if st.button("Vente/Échange", use_container_width=True, key="nav_vente"):
        set_current_page("Vente")
        st.rerun()
    if st.button("Lots", use_container_width=True, key="nav_lots"):
        set_current_page("Lots")
        st.rerun()
    if st.button("Historique", use_container_width=True, key="nav_historique"):
        set_current_page("Historique")
        st.rerun()
    if st.button("📊 Statistiques", use_container_width=True, key="nav_statistiques"):
        set_current_page("Statistiques")
        st.rerun()
    if st.button("🎰 Compteurs", use_container_width=True, key="nav_compteurs"):
        set_current_page("Compteurs")
        st.rerun()
    if st.button("Archivés", use_container_width=True, key="nav_archives"):
...
# ============================================================
    ...
    if not cd.get("lots"):
        ...
        if "lots_filter_value" not in st.session_state:
            st.session_state["lots_filter_value"] = "Tous"
        filter_cols = st.columns(len(filter_defs))
        for fcol, (filter_name, _) in zip(filter_cols, filter_defs):
            label = f"{filter_name} ({filter_counts[filter_name]})"
            btn_type = "primary" if st.session_state["lots_filter_value"] == filter_name else "secondary"
            if fcol.button(label, key=f"lot_filter_btn_{filter_name}", use_container_width=True, type=btn_type):
                st.session_state["lots_filter_value"] = filter_name
                st.rerun()
        selected_filter = st.session_state["lots_filter_value"]
        if selected_filter not in [name for name, _ in filter_defs]:
            selected_filter = "Tous"
            st.session_state["lots_filter_value"] = selected_filter
            ...
            with st.expander(expander_title, expanded=st.session_state.get(f"lot_expanded_{ix}", False)):
                ...
                (function() {{
                  ...
                  function makeSticky() {{
                    ...
                    if (!markerBlock) {{
                        if (attempts++ < 30) setTimeout(makeSticky, 200);
                        return;
                    }}
                    if (markerBlock.dataset.stickyWrapped === '1') return;
