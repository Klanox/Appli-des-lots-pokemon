...
def calc_cout_lot(lot, valeur_estimee=None, lot_idx=None):
    ...
    if valeur_estimee is None or valeur_estimee <= 0:
        stock_val = sum(
            float(c.get("suggested_price", 0.)) * card_available_qty(c)
            for c in cards
        )
        sold_cote = 0.
        for c in cards:
            for se in c.get("sold_entries", []):
                ...
                cote_unit = float(se.get("suggested_price_at_sale", 0.) or c.get("suggested_price", 0.) or 0.)
                if cote_unit <= 0:
                    cote_unit = float(se.get("price", 0.)) / max(qty, 1)
                sold_cote += cote_unit * qty
...
# ============================================================
    ...
    with st.expander("🔎 Détail du bénéfice du mois"):
        ...
        st.caption("Calcul actuel : coût = cote de la carte vendue ÷ valeur estimée du lot × prix d'achat du lot. Le bénéfice utilise le prix de vente réel.")
