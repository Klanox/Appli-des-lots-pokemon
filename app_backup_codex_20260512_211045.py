...
# ============================================================
    ...
    components.html("""
    <script>
    [50, 250, 700, 1500, 3000, 5000].forEach((delay) => {
        setTimeout(() => parent.window.scrollTo({top:0,left:0,behavior:'instant'}), delay);
    });
    </script>
    """, height=0)
        ...
        for display_ix,(ix,lt) in enumerate(visible_lots):
            ...
            with st.expander(expander_title, expanded=st.session_state.get(f"lot_expanded_{ix}", False)):
                ...
                setTimeout(() => {{
                    ...
                    if (!markerBlock || markerBlock.dataset.stickyWrapped === '1') return;
                    ...
                    wrapper.style.boxShadow = '0 8px 14px rgba(15,23,42,0.08)';
                    markerBlock.parentNode.insertBefore(wrapper, markerBlock.nextSibling);
                    let node = wrapper.nextSibling;
                    let moved = 0;
                    while (node && moved < 18) {{
                        const next = node.nextSibling;
                        const txt = node.innerText || '';
                        if (txt.includes('📦 Cartes du lot')) break;
                        wrapper.appendChild(node);
                        moved++;
                        node = next;
                    }}
                    markerBlock.dataset.stickyWrapped = '1';
                }}, 300);
                </script>
                """, height=0)
