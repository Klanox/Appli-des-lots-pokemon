...
APP_BUILD = "Codex 2026-05-23 mobile cloud prep"
...
def cloud_sync_enabled():
    return bool(_secret_value("SUPABASE_URL") and _secret_value("SUPABASE_KEY", "SUPABASE_ANON_KEY"))
...
def get_supabase_client():
    ...
    try:
        ...
        st.session_state["cloud_sync_error"] = f"Module Supabase absent: {e}"
        return None
    try:
        return create_client(
            _secret_value("SUPABASE_URL"),
            _secret_value("SUPABASE_KEY", "SUPABASE_ANON_KEY"),
        )
    except Exception as e:
        st.session_state["cloud_sync_error"] = f"Connexion Supabase impossible: {e}"
        return None
...
def search_in_cache(name, num=None):
    ...
    # 1c. Si un numéro est saisi, on ajoute aussi les variantes du même Pokémon :
    # ex: "Noctali 060" propose aussi "Noctali ex", "Noctali V", "Noctali VMAX" si elles existent.
    if num:
        for idx_name, idx_norm, cards in iter_cache_items():
            if idx_norm == name_norm:
                continue
...
def recent_sale_notes_for_card(name, num=None, limit=3):
    """Retourne les derniers prix de vente connus pour une carte déjà vendue."""
    if not name:
        return []
...
with st.sidebar:
    ...
    if cloud_sync_enabled():
        st.caption("☁️ Synchronisation cloud prête")
    ...
    elif st.session_state.get("cloud_sync_error"):
        st.caption(st.session_state.get("cloud_sync_error"))
    sts=gst()
        ...
        function syncLotHeaders() {
            ...
            if (addMarker) {
                ...
                if (lotBlock && markerChild) {
                    ...
                    shield.style.setProperty('top', '48px', 'important');
                    ...
                    shield.style.setProperty('border-radius', '0 0 20px 20px', 'important');
                    formParts.forEach(function(part) {
                        ...
                        part.style.setProperty('padding', '0.25rem 1.15rem', 'important');
                        ...
                        if (part === formParts[0]) {
                            ...
                            part.style.setProperty('padding-top', '1.15rem', 'important');
                            part.style.setProperty('box-shadow', 'inset 0 1px 0 rgba(15,23,42,0.10)', 'important');
                            part.querySelectorAll('[data-testid="stVerticalBlock"], [data-testid="stHorizontalBlock"], [data-testid="stElementContainer"]').forEach(function(inner) {
                        ...
                        if (part === formParts[formParts.length - 1]) {
                            ...
                            part.style.setProperty('padding-bottom', '1.15rem', 'important');
                            part.style.setProperty('box-shadow', 'inset 0 -1px 0 rgba(15,23,42,0.10)', 'important');
                            part.querySelectorAll('[data-testid="stVerticalBlock"], [data-testid="stHorizontalBlock"], [data-testid="stElementContainer"]').forEach(function(inner) {
                    ...
                    shield.style.setProperty('height', Math.max(topOffset - 48 + 14, 1) + 'px', 'important');
                }
            }
        }
        syncLotHeaders();
